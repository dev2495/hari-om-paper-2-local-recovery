...
- Local product HEAD: `1e397887c5bf1d88d7d22c014f5bbd8dd1eda4ba` (capacity/GRN-pin/PLAN-PUR-QC tests). Started from `5187a640ac86123806d79b0160f022346a3745a5`; no force-push, no discarded work.
...
- Original 56/192 restored at `docs/review/baseline-v2/`. Overlay this cycle: **PASS 28 / PARTIAL 29 / LIMITATION 3 / NOT_RUN 132**.
...
- Isolated production HTTP pid **70283** on 28004 and inventory pid **70284** on 28005 after this wave’s source change. JWT hash prefix `c0f8ce9c6baa035a` matches auth. Foreign 13000 left running. Next BUILD_ID still `Yz4l4-NEecxcN4k1Xtf_H`.
...
1. Remaining `PUR-06`/`PUR-07`/`PUR-08`, then `QC-03`–`QC-10`, then `QCT-016+` / `INC-*` / `REG-01`/`REG-02`/`REG-04`. Close remaining PARTIAL only when the full original procedure is proven (`COMM-08` live order+UI, `REL-11` list/detail/bulk/legacy, `PLAN-04` group-move of started/dispatched, `PLAN-05` supplier+inventory, `PLAN-07` live holiday placement, `PLAN-08` live two-then-third oven jobs, `PUR-01` tool SKU, `PUR-05` rejected remainder).
