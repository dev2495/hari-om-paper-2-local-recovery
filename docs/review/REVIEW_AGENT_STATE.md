...
- Local product this wave: `1943b30` QCT-062 required instrument evidence controls readiness, on parents overlay `0edff86` / product `279dfc0`. No force-push, no discarded work.
- Served Next on 23000: BUILD_ID `EYFrs5GhOyHNGtioE2__r` (npm parent **32080**, next **32109**). Prior `_opGJjeYOkhKoqBiWqi2C` does **not** bind this bundle.
- Focused Chromium on `EYFrs5GhOyHNGtioE2__r`: QCT-062 **1 passed / 0 failed** in 4.9s. Uncontended full Chromium **48 passed / 0 failed** in 2.5m; auth `:28001` stayed up. One uninterrupted 48/48. That is the Chromium project suite, **not** original COMM/REL pack PASS.
...
- Original 56/192 restored at `docs/review/baseline-v2/`. Overlay this cycle: **PASS 95 / PARTIAL 21 / LIMITATION 3 / NOT_RUN 73**.
- Requirements overlay PASS: `R12`, `R13`, `R14`, `R15`, `R19`, `QCR-03`, `QCR-04`, `QCR-06`, `QCR-07`, `QCR-08`, `QCR-09`, `QCR-10`, `QCR-12`, `QCR-13`, `QCR-14`, `QCR-17`. `QCR-34` stays PARTIAL (`QCT-110` still NOT_RUN). `QCR-16` stays PARTIAL (`QCT-102`/`QCT-116` still NOT_RUN). `QCR-32` stays PARTIAL. `QCR-24` is PARTIAL (`QCT-084`/`QCT-105` still NOT_RUN). `QCR-21` and `QCR-29` are PARTIAL. `QCR-11` is PARTIAL (`QCT-126` still NOT_RUN). `QCR-05` stays PARTIAL. `QCR-19` is PARTIAL (`QCT-063`/`QCT-064` NOT_RUN). `QCR-31` stays PARTIAL. CODE is not PASS. GROSS_ESTIMATE and PARTIAL_REJECTION_UNSUPPORTED remain honest limitations (`DEM-02`, `DEM-06`, `QCT-107`).
...
- Isolated pids this wave: Next **32109** :23000 (npm parent **32080**), BFF **75339** :24000, production **31662** :28004 (venv-verify, JWT_SECRET len 23, MASTERDATA_SERVICE_URL `http://127.0.0.1:28002`, SALES_SERVICE_URL `http://127.0.0.1:28008`), inventory **2331** :28005, sales **2337** :28008, auth **9821** :28001, master **90295** :28002, spec **31660** :28003, analytics **13483** :28007. Foreign 13000 pid 69663 left running.
- Schema: no new tables. Instrument requirement lives on frozen QC profile parameters; documented evidence lives on inspection readings JSON. Missing-setup flag remains on existing `job_cards.spec_snapshot` JSONB.
...
- Original QCT-062 (not the title): Submit readings requiring calibrated instrument using missing/expired instrument then corrected documented evidence. Expected: Required instrument evidence controls readiness; no invented calibration or silent PASS.
- Live + unit: missing 409 INVALID_INSTRUMENT, expired 409, status-only valid without due/certificate is not invented calibration, documented in-cal evidence then PASS. QCT-060 regression green.
- Chromium QCT-062: **1 passed** focused and again as test 31 of the uninterrupted 48/48 full run on BUILD_ID `EYFrs5GhOyHNGtioE2__r`.
- Overlay: **PASS**. `QCR-34` stays PARTIAL until `QCT-110`.
...
1. Pack-order **QCT-063+** (save a signed/recorded sample with multiple failures and retry request — one appropriate case/hold per scope/round, retaining all failures; no duplicated quantities/cases on replay). Prove the original procedure, not the title.
