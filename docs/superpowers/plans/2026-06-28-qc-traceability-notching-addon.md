...
- 2026-06-28: Railway deploy surfaced an existing auth-service startup blocker: default plant seeding checked only plant code and could insert the fixed Plant A UUID again. Seeder now checks both code and UUID; `seed_default_plants()` was run twice locally without duplicate-key failure.
