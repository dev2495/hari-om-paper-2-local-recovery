...
function eventTone(eventType: string) {
  if (eventType === "SCRAP") return "text-signal-rose-ink"
  if (eventType === "MAINTENANCE") return "text-signal-amber-ink"
  if (eventType === "PRODUCTION_USED") return "text-signal-cyan-ink"
    ...
    <div className="space-y-5 px-6 pb-10 pt-2" data-testid="tooling-report-page">
      ...
      <div className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
        <Panel
          ...
          actions={
            <Link href="/masters/tools" className="text-sm font-semibold text-signal-cyan-ink hover:underline">
              Manage tools
            </Link>
          }
        >
          {isLoading ? (
            <div className="py-6 text-sm text-muted-foreground">Loading tooling report...</div>
          ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {byCategory.map((row, index) => (
                    <tr key={`${row.category}-${row.status}-${index}`} className="border-b border-border">
                      <td className="py-2 pr-3 font-medium text-foreground">{CATEGORY_LABELS[row.category] || row.category}</td>
                      <td className="py-2 pr-3 text-muted-foreground">{row.status}</td>
                      <td className="py-2 pr-3 text-right font-semibold text-foreground">{formatNumber(Number(row.count || 0))}</td>
                      <td className="py-2 pr-3 text-right text-muted-foreground">{formatNumber(Number(row.usage_count || 0))}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        >
          <div className="space-y-3">
            {usage.slice(0, 10).map((row, index) => (
              <div key={`${row.category}-${row.tool_name}-${row.event_type}-${index}`} className="rounded-2xl border border-border bg-muted px-4 py-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="text-sm font-semibold text-foreground">{CATEGORY_LABELS[row.category] || row.category}: {row.tool_name}</p>
                    <p className={`mt-1 text-xs font-semibold ${eventTone(row.event_type)}`}>{row.event_type}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-lg font-semibold text-foreground">{formatNumber(Number(row.count || 0))}</p>
                    <p className="text-xs text-muted-foreground">{formatNumber(Number(row.production_qty || 0))} planned pcs</p>
                  </div>
                </div>
                <p className="mt-2 text-xs text-muted-foreground">Last event {formatDate(row.last_used_at)}</p>
              </div>
            ))}
            {!usage.length ? (
              <div className="rounded-2xl border border-border bg-card px-4 py-6 text-sm text-muted-foreground">No tooling usage has been logged yet.</div>
            ) : null}
          </div>
        </Panel>
      </div>
      ...
      <Panel eyebrow="Blade accountability" title="Lifetime output by physical tool" description="Lifetime accepted and scrap output for each permanent QR asset. Use the cycle register below for output between grinding returns.">
        {assetsLoading ? <div className="py-6 text-sm text-muted-foreground">Loading asset output...</div> : <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead><tr className="border-b border-border text-left text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground"><th className="py-2 pr-3">Asset / QR</th><th className="py-2 pr-3">Tool</th><th className="py-2 pr-3">Category</th><th className="py-2 pr-3">Version</th><th className="py-2 pr-3 text-right">Usage runs</th><th className="py-2 pr-3 text-right">Good output</th><th className="py-2 pr-3 text-right">Scrap output</th><th className="py-2">Status</th></tr></thead><tbody>{assetOutput.map((row) => <tr key={row.asset_no} className="border-b border-border"><td className="py-2 pr-3"><p className="font-semibold text-foreground">{row.asset_no}</p><p className="text-xs text-muted-foreground">{row.qr_value}</p></td><td className="py-2 pr-3">{row.definition_name}</td><td className="py-2 pr-3">{CATEGORY_LABELS[row.category] || row.category}</td><td className="py-2 pr-3">V{row.grind_version || 0}</td><td className="py-2 pr-3 text-right">{formatNumber(Number(row.usage_count || 0))}</td><td className="py-2 pr-3 text-right font-semibold text-signal-emerald-ink">{formatNumber(Number(row.produced_qty || 0))}</td><td className="py-2 pr-3 text-right text-signal-rose-ink">{formatNumber(Number(row.scrap_qty || 0))}</td><td className="py-2">{row.status}</td></tr>)}</tbody></table>{!assetOutput.length ? <div className="py-5 text-sm text-muted-foreground">No inwarded physical tools yet.</div> : null}</div>}
      </Panel>
      ...
      <Panel eyebrow="Grinding trace" title="Output by blade grinding version" description="Each grinding return starts the next version. This register preserves production completed before and after every sharpening cycle.">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[900px] text-sm">
            ...
            <tbody>{grindCycles.map((row) => <tr key={`${row.asset_no}-${row.grind_version}`} className="border-b border-border"><td className="py-2 pr-3"><p className="font-semibold text-foreground">{row.asset_no}</p><p className="text-xs text-muted-foreground">{row.qr_value}</p></td><td className="py-2 pr-3">{row.definition_name}</td><td className="py-2 pr-3 font-semibold">Grinding V{row.grind_version || 0}</td><td className="py-2 pr-3 text-right">{formatNumber(Number(row.usage_count || 0))}</td><td className="py-2 pr-3 text-right font-semibold text-signal-emerald-ink">{formatNumber(Number(row.produced_qty || 0))}</td><td className="py-2 pr-3 text-right text-signal-rose-ink">{formatNumber(Number(row.scrap_qty || 0))}</td><td className="py-2 text-muted-foreground">{formatDate(row.last_used_at)}</td></tr>)}</tbody>
          </table>
          {!grindCycles.length ? <div className="py-5 text-sm text-muted-foreground">Cycle output will appear after an issued physical tool is used on a completed job-card stage.</div> : null}
        </div>
      </Panel>
            ...
            <tbody>
              {recentLogs.map((log) => (
                <tr key={log.id} className="border-b border-border hover:bg-muted">
                  ...
                  <td className="py-2 pr-3 text-muted-foreground">{CATEGORY_LABELS[log.category] || log.category}</td>
                  ...
                  <td className="py-2 pr-3 text-muted-foreground">{log.notes || "-"}</td>
                </tr>
              ))}
            </tbody>
          </table>
          {!recentLogs.length ? (
            <div className="mt-3 flex items-center gap-2 rounded-xl border border-signal-amber-line bg-signal-amber-soft px-3 py-2 text-sm text-signal-amber-ink">
