    ...
    <div className="space-y-5 px-6 pb-10 pt-2" data-testid="reports-operations-page">
      ...
      <ReportFilterBar>
        <FilterField label="Window">
          <span className="rounded-md border border-border bg-muted px-2 py-1 text-sm font-semibold text-muted-foreground">Last 7 days</span>
        </FilterField>
        ...
        {machines.length > 1 ? (
          <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-3 text-xs">
            {machines.slice(1).map((m) => (
              <div key={m.key} className="rounded-xl border border-border bg-muted px-3 py-2">
                <div className="flex items-center justify-between">
                  <span className="font-semibold text-foreground">{m.label}</span>
                  <span className="text-muted-foreground">{formatNumber(m.total_hours)} h</span>
                </div>
                <p className="mt-1 text-muted-foreground">Peak hour load: {formatNumber(m.peak_hour_load)}</p>
              </div>
            ))}
          </div>
        ) : null}
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)]">
        ...
        <Panel eyebrow="Adherence" title="Schedule adherence" description="% of stages completed within the planned cycle.">
          <div className="text-center">
            <p className="text-5xl font-extrabold text-foreground">{adherencePercent === null ? "—" : formatPct(adherencePercent)}</p>
      ...
      >
        {operatorRows.length ? (
          <div className="overflow-x-auto"><table className="w-full text-sm">
            ...
            <tbody>
              {operatorRows.map((o) => (
                <tr key={o.operator} className="border-b border-border">
                  <td className="py-2 pr-3 font-medium text-foreground">{o.operator}</td>
                  <td className="py-2 pr-3 text-muted-foreground">{o.primary_stage}</td>
            ...
            <tbody>
              {blockedRows.slice(0, 12).map((row: any, i: number) => (
                <tr key={i} className="border-b border-border">
                  ...
                  <td className="py-2 pr-3 text-signal-rose-ink">{row.blocker_reason || row.reason || "Waiting"}</td>
                ...
                <tbody>
                  {((lag as any).laggard_rows || []).slice(0, 8).map((r: any, i: number) => (
                    <tr key={i} className="border-b border-border">
                      ...
                      <td className={`py-2 pr-3 text-right font-bold ${Number(r.lag_minutes) > 360 ? "text-signal-rose-ink" : Number(r.lag_minutes) > 60 ? "text-signal-amber-ink" : "text-muted-foreground"}`}>
                  ...
                  <tbody>
                    {shortCloses.slice(0, 8).map((s: any) => (
                      <tr key={s.id} className="border-b border-border">
                        ...
                        <td className="text-right font-bold text-signal-rose-ink">{Math.round(Number(s.gap_qty || 0))}</td>
