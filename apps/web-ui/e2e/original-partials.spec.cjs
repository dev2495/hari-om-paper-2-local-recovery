...
test("QCT-046 blank multi-page print keeps samples, paired oven, writable spaces, and no default PASS", async ({ page }) => {
  ...
  await expect(page.getByTestId("print-process-sample")).toHaveCount(3)
  ...
  const ovenBreak = await page.getByTestId("print-page-process").evaluate((el) => getComputedStyle(el).breakAfter || getComputedStyle(el).pageBreakAfter)
...
test("QCT-058 retrospective measured time after later stage and dispatch is a late exception without retroactive claims", async ({ page }) => {
  test.setTimeout(180_000)
  const assertCritical = beginCriticalMonitoring(page)
  const fixture = getBrowserFixture()
  const seeded = spawnProductionPytest("tests/test_original_qct058_live.py::test_qct058_ui_job_seed")
  expect(seeded.status, seeded.stderr || seeded.stdout).toBe(0)
  const artifact = JSON.parse(fs.readFileSync(path.join(workspaceRoot, "reports", "qct058-ui-job.json"), "utf8"))
  const jobId = String(artifact.job_id)
  await cookieLogin(page, fixture.auth.admin_email, fixture.auth.admin_password, fixture.plants.plant_a.id)
  await selectSeededQualityJob(page, jobId)
  await page.getByTestId("quality-stage-tab-WINDER").click()
  await page.getByTestId("quality-stage-measured-at").fill(String(artifact.measured_at_local))
  await page.getByTestId("stage-qc-sample-id").fill("QCT058-UI")
  await page.getByTestId("stage-qc-reading-id").fill("77")
  await page.getByTestId("stage-qc-reading-od").fill("91")
  await page.getByTestId("stage-qc-reading-height").fill("90")
  await page.getByTestId("stage-qc-reading-weight").fill("250")
  await page.getByTestId("stage-qc-reading-cs").fill("100")
  await expect(page.getByTestId("stage-qc-reason-height")).toBeVisible()
  await page.getByTestId("stage-qc-reason-height").fill("paper card height measured short before oven")
  await page.getByTestId("quality-stage-submit").click()
  await expect(page.getByTestId("quality-stage-verdict")).toHaveText("FAIL")
  const late = page.getByTestId("late-quality-exception")
  await expect(late).toBeVisible()
  await expect(late).toContainText(/Late quality exception/i)
  await expect(late).not.toContainText(/prevented the (dispatch|shipment)/i)
  const measuredText = await page.getByTestId("qc-measured-at").innerText()
  const recordedText = await page.getByTestId("qc-recorded-at").innerText()
  expect(measuredText).not.toBe(recordedText)
  await expect(page.getByTestId("surviving-stock")).toBeVisible()
  await expect(page.getByTestId("earlier-shipment")).toContainText(/SEALED/)
  const plantHeaders = { "X-Plant-ID": fixture.plants.plant_a.id }
  const listed = await page.request.get(`/api/production/quality/inspections?job_card_id=${jobId}`, {
    headers: plantHeaders,
  })
  ...
  const rows = await listed.json()
  expect(rows[0].status).toBe("FAIL")
  expect(rows[0].late_quality_exception).toBeTruthy()
  expect(rows[0].measured_at).not.toBe(rows[0].recorded_at)
  expect(rows[0].retroactive_prevention_claimed).toBeFalsy()
  expect(rows[0].earlier_shipments.some((row) => row.status === "SEALED")).toBeTruthy()
  await assertCritical()
})
