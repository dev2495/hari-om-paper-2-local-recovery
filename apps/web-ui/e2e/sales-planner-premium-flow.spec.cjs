...
test("premium sales and planner surfaces load with animated interactive elements", async ({ page }) => {
  ...
  await expect(page.getByRole("heading", { name: /^WINDER_02$/ }).first()).toBeVisible()
  await expect(page.getByRole("heading", { name: /^WINDER_03$/ }).first()).toBeVisible()
  await expect(page.getByRole("combobox", { name: /^machine$/i })).toContainText(/WINDER_02/)
  await expect(page.getByRole("combobox", { name: /^machine$/i })).toContainText(/WINDER_03/)
