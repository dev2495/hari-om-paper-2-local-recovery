...
export function InventoryLabelPrint({ label, title = "Inventory Label Preview" }: Props) {
  ...
  return (
    <section className="rounded-lg border border-border bg-card p-4 shadow-sm print:border print:border-slate-800 print:shadow-none">
      <div className="no-print mb-3 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <span className="inline-flex h-8 w-8 items-center justify-center rounded-lg bg-signal-cyan-soft text-signal-cyan-ink">
            <QrCode className="h-4 w-4" />
          </span>
          <div>
            <h2 className="text-sm font-semibold text-foreground">{title}</h2>
            <p className="text-xs text-muted-foreground">{entityType} · {label.stock_status || "status pending"}</p>
          </div>
        </div>
        <button
          type="button"
          onClick={() => window.print()}
          className="inline-flex h-9 items-center gap-2 rounded-lg border border-border bg-card px-3 text-sm font-semibold text-muted-foreground hover:bg-muted"
        >
