...
export function CommandCenter({ role, testId, variant = "landing", header }: { role: LandingRole; testId?: string; variant?: "landing" | "analytics"; header?: ReactNode }) {
  ...
  const kpis: Record<KpiKey, { label: string; value: string; detail: string; icon: LucideIcon; tone: MetricTone; href?: string; spark?: number[]; progress?: number | null; ready: boolean }> = {
    ...
    utilization: { label: "Machine load", value: pct(Number(pack.production?.summary?.avg_machine_utilization_percent)), detail: "Assigned load vs rated capacity, top machines", icon: Workflow, tone: "violet", progress: Number(pack.production?.summary?.avg_machine_utilization_percent), href: "/reports/production", ready: packReady },
              ...
              return (
                <div key={row.machine_id}>
                  <div className="flex items-center justify-between gap-2 text-[12.5px]">
                    ...
                    <span className={cn("font-semibold tabular-nums", value > 100 ? "text-signal-rose-ink" : value > 85 ? "text-signal-amber-ink" : "")}>{pct(value)}</span>
                  </div>
                  <div className="mt-1 h-2 overflow-hidden rounded-full bg-muted">
