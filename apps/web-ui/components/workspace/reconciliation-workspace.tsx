...
export function ReconciliationWorkspace({ view = "workspace" }: { view?: "workspace" | "actuals" | "drift" | "history" }) {
  const { activePlant } = useAuth()
  const [month, setMonth] = useState(currentMonth())
  const [weekStart, setWeekStart] = useState(currentWeekStart())
  const activeTab = view
  const router = useRouter()
  const setActiveTab = (next: TabKey) => router.push(next === "workspace" ? "/production/reconciliation" : `/production/reconciliation/${next}`)
  ...
  return (
    <div className="space-y-6 animate-enter-up" data-testid="reconciliation-page">
        ...
        actions={
          <>
            <input
              ...
              className="h-9 rounded-xl border border-border/70 bg-card/90 px-3 text-sm font-semibold shadow-sm"
            />
            <span
              className={cn(
      ...
      {books?.locked_through && (
        <div className="rounded-2xl border border-signal-amber-line bg-signal-amber-soft/80 px-4 py-3 text-sm font-semibold text-signal-amber-ink shadow-sm">
          ...
          <ul className="space-y-2.5">
            {blockers.map((b, idx) => (
              <li
                ...
                className="flex items-start gap-3 rounded-2xl border border-signal-rose-line bg-card px-4 py-3 shadow-sm"
              >
                <span className="mt-0.5 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-signal-rose-soft text-signal-rose-ink">
                  <AlertTriangle className="h-3.5 w-3.5" />
                </span>
              ...
              <div className="flex items-center gap-2">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="h-9 w-64 rounded-xl border border-border bg-card pl-9 pr-3 text-sm shadow-sm"
                  />
                </div>
              </div>
            }
                    ...
                    {filteredRows.length === 0 ? (
                      ...
                      filteredRows.map((row) => {
                        const status = row.over_tolerance
                          ...
                          : "emerald"
                        return (
                          <tr key={row.item_code} className="border-t border-border transition hover:bg-signal-cyan-soft/30">
                              ...
                              <span
                                className={cn(
                                  ...
                                  status === "emerald"
                                    ...
                                    : status === "amber"
                                    ? "border-signal-amber-line bg-signal-amber-soft text-signal-amber-ink"
                                    : "border-signal-violet-line bg-signal-violet-soft text-signal-violet-ink",
                                )}
                              >
                                {row.over_tolerance
                                  ? row.needs_explanation
                            ...
                            <td className="px-3 py-2.5 max-w-[220px]">
                              <span className="block truncate text-[12px] text-muted-foreground" title={row.notes || ""}>
            ...
            >
              <div className="space-y-4">
                <textarea
                  ...
                  disabled={!canApprove}
                  className="w-full rounded-xl border border-border bg-card p-3 text-sm shadow-sm disabled:bg-muted disabled:text-muted-foreground"
                />
                <button
                ...
                {approveMutation.isSuccess && (
                  <div className="rounded-2xl border border-signal-emerald-line bg-signal-emerald-soft p-3 text-[12.5px] font-semibold text-signal-emerald-ink">
                      ...
                      <tr key={row.item_code} className="border-t border-border">
                        ...
                        <td className="px-3 py-2.5 text-right">
                          <input
                            ...
                            className="h-9 w-28 rounded-lg border border-border bg-card px-2 text-right tabular-nums text-sm disabled:bg-muted"
                          />
                        </td>
                        <td className="px-3 py-2.5 text-right">
                          <input
                            ...
                            className="h-9 w-32 rounded-lg border border-border bg-card px-2 text-right tabular-nums text-sm disabled:bg-muted"
                          />
                        </td>
                        <td className="px-3 py-2.5">
                          <input
          ...
          <Panel
            ...
            actions={
              <input
                ...
                className="h-9 rounded-xl border border-border bg-card px-3 text-sm font-semibold shadow-sm"
              />
            }
          >
                  ...
                  <tbody>
                    {(Array.isArray(driftQuery.data?.rows) ? driftQuery.data!.rows : []).length === 0 ? (
                      ...
                      (driftQuery.data!.rows as any[]).map((row: any) => (
                        <tr key={row.item_code} className="border-t border-border hover:bg-signal-cyan-soft/30">
                          <td className="px-4 py-2.5">
                            <span className="block font-mono text-[12px] font-bold text-signal-cyan-ink">{row.item_code}</span>
                            <span className="block text-[11px] text-muted-foreground">{row.item_name || "—"}</span>
                          </td>
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <tbody>
                {(Array.isArray(historyQuery.data?.rows) ? historyQuery.data!.rows : []).map((row: any) => (
                  <tr key={`${row.plant_id}:${row.month_start}`} className="border-t border-border">
                    <td className="px-4 py-2.5 font-semibold text-foreground">{dayjs(row.month_start).format("MMM YYYY")}</td>
                    <td className="px-3 py-2.5">
                      <span
                        className={cn(
                    ...
                    <td className="px-3 py-2.5 text-muted-foreground">{row.approved_by || "—"}</td>
                    <td className="px-3 py-2.5 text-muted-foreground">{row.approved_at ? dayjs(row.approved_at).format("DD MMM YYYY HH:mm") : "—"}</td>
                  </tr>
                ))}
