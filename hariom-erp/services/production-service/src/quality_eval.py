...
READING_ALIASES: dict[str, tuple[str, ...]] = {
    ...
    "pre_weight": ("pre_weight", "pre-weight", "pre_oven_weight_kg"),
    "post_weight": ("post_weight", "post-weight", "post_oven_weight_kg"),
...
def evaluate_parameter(rule: ParameterRule, reading: Any, reason: Optional[dict[str, Any]] = None) -> ParameterResult:
    ...
    if rule.input_type in {"text", "select", "boolean"}:
        if reading in (None, ""):
        ...
        return ParameterResult(
            ...
            submitted=reading,
            rule=rule,
            message="Descriptive observation; text cannot establish a measured PASS.",
            reason=reason,
        )
...
def evaluate_stage(
    *,
    ...
    if stage_key == STAGE_OVEN:
        ...
        if post_present and not pair_id:
            results.append(
                ParameterResult(
                    code="sample_id",
                    label="Oven sample",
