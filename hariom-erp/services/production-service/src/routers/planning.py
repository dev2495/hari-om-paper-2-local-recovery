...
NOTCH_TOOL_FIELD_CATEGORY_MAP = {
    "notch_type": ("NOTCH", "Notch"),
    "notching_blade": ("BLADE", "Blade"),
    "notching_holder": ("HOLDER", "Holder"),
...
def _build_document_snapshot(
    ...
    return {
        "header": {
            ...
            "parchment_resolution": spec_snapshot.get("parchment_resolution") or "ALIGNED",
            "parchment_conflict": spec_snapshot.get("parchment_conflict") or None,
            "approved_parchment_color": spec_snapshot.get("parchment_color"),
            "ordered_parchment_color": spec_snapshot.get("sales_order_line_parchment_color"),
        ...
        "setup_tooling": {
            ...
            "v_flat": spec_snapshot.get("v_flat") or "",
...
def capture_stage_output(
    ...
    if save_mode == "draft":
        ...
        return StageActionResponse(
            ...
            remaining_open_segments=len(_open_stage_segments(db, job_card.id, selected_stage)),
            entry_saved=True,
            warnings=hold_warnings + machine_warnings + time_warnings,
            reel_issue_ids=[str(value) for value in (stage.reel_issue_ids or [])],
            quality_hold_ids=[str(hold.id) for hold in active_holds],
        )
    ...
    if active_holds and not override_reason:
        ...
        return StageActionResponse(
            ...
            remaining_open_segments=len(_open_stage_segments(db, job_card.id, selected_stage)),
            entry_saved=True,
            warnings=warnings + machine_warnings + time_warnings,
            reel_issue_ids=[str(value) for value in (stage.reel_issue_ids or [])],
            quality_hold_ids=[str(hold.id) for hold in active_holds],
        )
    ...
    return StageActionResponse(
        ...
        override_reason=override_reason,
        warnings=machine_warnings + time_warnings,
        reel_issue_ids=[str(value) for value in (stage.reel_issue_ids or [])],
        quality_hold_ids=[str(hold.id) for hold in quality_holds],
    )
